...
rules = {
 'R0 signal days only: QQQ RSI2<10, dip<=-3% (tested rule)': base & (ret1 <= -0.03) & (q_rsi.lt(10).values[:, None]),
 'R1 EVERY day: dip<=-3%, >SMA200 (same filter, no market condition)': base & (ret1 <= -0.03),
 'R6 R1 on NON-signal days only (what tier B would be)': base & (ret1 <= -0.03) & (q_rsi.ge(10).values[:, None]),
 'R2 EVERY day: 5d pullback<=-8%, >SMA200': base & (ret5 <= -0.08),
 'R3 EVERY day: stock RSI2<5, >SMA200': base & (srsi < 5),
 'R7 EVERY day: close<SMA10, 5d<=-5%, >SMA200': base & (C < sma10) & (ret5 <= -0.05),
 'R8 EVERY day: close<20d low, >SMA200': base & (C < ll20),
 'R4 dip<=-3% on QQQ down days (q_ret<0)': base & (ret1 <= -0.03) & (q_ret.lt(0).values[:, None]),
 'R5 dip<=-3% when QQQ RSI2<30': base & (ret1 <= -0.03) & (q_rsi.lt(30).values[:, None]),
 'R9 dip<=-3% when QQQ RSI2<20': base & (ret1 <= -0.03) & (q_rsi.lt(20).values[:, None]),
 'R10 EVERY day: dip<=-3%, >SMA200, stock RSI2<10': base & (ret1 <= -0.03) & (srsi < 10),
 'R11 EVERY day: ret1>=+3% (momentum, top-2 liquid)': base & (ret1 >= 0.03),
}
TOP = 2
def evaluate(mask):
